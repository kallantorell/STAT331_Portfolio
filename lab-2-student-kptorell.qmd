...
nrow(surveys)
```
#the dimensions of this data is 30463 rows and 15 columns
...
ggplot(data = surveys, aes(x = weight, y = species)) +
  ...
  labs(
    x = "Weight (g)",
    y = "Species",
    title = "Distribution of Species by Weight"
  ) + 
...
Based on the results of the ANOVA F-test, the test shows a very large test F statistic of F = 19916, and a p-value of less than 2e^-16. This p-value is much smaller than the 0.05 significance level so we can reject the null hypothesis. In conclusion, there are statistically significant differences in th emean weight upon the rodent species, and at least one species has a varying average weight compared to the others. 
